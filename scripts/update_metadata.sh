...
function usage {
    ...
    echo ""
    echo "usage: $programname --basex_path string --db_name string --resource_id string --project_dir_path string"
    echo ""
    ...
    echo "  --resource_id           absolute path to import folder"
    echo "                          (example: theater)"
...
bash "$basex_path/basex" -b dbName=$db_name -b resourceId=$resource_id scripts/update_metadata.xq
